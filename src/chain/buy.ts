 ...
 */
export async function buyCard(cardId: BigNumber): Promise<void> {
    ...
    const makerAssetData = assetDataUtils.encodeERC721AssetData(dummyERC721TokenContract.address, cardId);
    const takerAssetData = assetDataUtils.encodeERC20AssetData(etherTokenAddress);

    // Mint a new ERC721 token for the maker
    const mintTxHash = await dummyERC721TokenContract.mint.sendTransactionAsync(maker, cardId, { from: maker });
    console.log('card was minted for maker', mintTxHash);
    ...
    console.log('allow', makerERC721ApprovalTxHash);
